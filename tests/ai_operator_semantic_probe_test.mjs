...
test('subscriber reply path keeps behavior tunable while truth rules stay invariant', () => {
  ...
  assert.match(source, /normalizeLabBehavior\(behavior\)/);
  assert.match(source, /behaviorInstruction\(profile\)/);
  assert.doesNotMatch(source, /profile\.confidenceStyle|profile\.curiosity|profile\.skepticism|profile\.brevity|profile\.maxFollowUpQuestions/);
});
