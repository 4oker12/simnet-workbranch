...
assert.equal(
  contentScripts.includes('src/features/call/realtime/current-call-resolver.js'),
  false,
  'opening registration must use one filtered GET, not a background call-list poller'
);
assert.match(ui, /fresh:\s*true/);
assert.match(ui, /forceRefresh:\s*true/);
assert.match(ui, /activityKnown/);
assert.match(ui, /activityActive/);
assert.match(ui, /call\.call_list_fetch/);
assert.match(ui, /call\.form_fetch/);
assert.match(ui, /call\.form_parse/);
assert.match(ui, /call\.focus_select/);
assert.match(ui, /call\.form_render/);
assert.match(background, /focusKind/);
assert.match(background, /focusCallKey/);
assert.match(activeGuard, /directFilteredLiveCall\(registration\) \|\| currentLiveCall\(\)/);
assert.match(activeGuard, /latest completed call instead of a false LIVE call/);
assert.match(activeGuard, /fresh native phone state is stronger than the slower call_list DOM/);
assert.match(activeGuard, /if \(nativeStateRecent\(\)\) return activeState\?\.active === true/);
