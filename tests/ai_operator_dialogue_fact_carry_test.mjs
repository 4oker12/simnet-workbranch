...
const facts = augmentRequiredFactsForTurn({ analysis: correction, transcript: [{ role:'customer', text:'Ой, оптоволокно имел в виду, слово перепутал.' }], labState });
assert.ok(facts.includes('subscriber.access.connectionFamily'), 'CORRECT must carry parent required fact before resolver');
...
const finance = augmentRequiredFactsForTurn({ analysis:{probe:{requiredFacts:[]}}, transcript:[{role:'customer',text:'на сколько месяцев хватит денег?'}], labState:{} });
