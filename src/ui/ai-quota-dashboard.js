...
function ensureStyle() {
  ...
  style.textContent = `
    ...
    .ai-quota-head{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:10px}.ai-quota-head strong{display:block;font-size:14px}.ai-quota-head span{font-size:11px;opacity:.7;text-align:right}
    .ai-quota-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(285px,1fr));gap:9px}.ai-quota-card{padding:10px;border:1px solid rgba(92,40,70,.14);border-radius:10px;background:rgba(255,255,255,.82)}
    ...
    .ai-quota-row{margin-top:8px}.ai-quota-label{display:flex;justify-content:space-between;gap:8px;font-size:11px;margin-bottom:3px}.ai-quota-label b{font-variant-numeric:tabular-nums}.ai-quota-meta{font-size:9px;opacity:.65;margin-top:2px}
    .ai-quota-track{height:8px;border-radius:999px;background:rgba(80,40,60,.10);overflow:hidden}.ai-quota-fill{height:100%;border-radius:inherit;background:#a50046;transition:width .15s ease}.ai-quota-fill.warn{background:#b56a00}.ai-quota-fill.danger{background:#b42318}
    .ai-quota-empty{font-size:11px;opacity:.65;padding:6px 0}.ai-quota-saving{margin-top:10px;padding-top:8px;border-top:1px solid rgba(92,40,70,.10);font-size:10px;opacity:.72}.ai-quota-shrink{margin-top:8px;padding:6px 8px;border-radius:7px;background:rgba(165,0,70,.06);font-size:10px;font-variant-numeric:tabular-nums}
  `;
  document.head.append(style);
}

function metricRow(name, usedValue, limitValue, meta = '') {
  ...
  note.textContent = `осталось ${integer(remaining)}${meta ? ` · ${meta}` : ''}`;
...
function modelCard(model, limits, telemetry = {}) {
  ...
  const minute = events.filter(item => number(item?.at) >= Date.now() - 60000);
  ...
  const hasTpmHeader = rate.limitTokens != null && rate.remainingTokens != null;
  const headerTpmLimit = number(rate.limitTokens) || limits.tpm;
  const headerTpmUsed = hasTpmHeader ? Math.max(0, headerTpmLimit - number(rate.remainingTokens)) : localMinuteTokens;
  ...
  const hasRpdHeader = rate.limitRequests != null && rate.remainingRequests != null;
  const headerRpdLimit = number(rate.limitRequests) || limits.rpd;
  const headerRpdUsed = hasRpdHeader ? Math.max(0, headerRpdLimit - number(rate.remainingRequests)) : localDayRequests;
  const rpd = providerMetric(rate, 'rpd', headerRpdUsed, headerRpdLimit);

  card.append(
    metricRow('TPM · токены/мин', tpm.used, tpm.limit, tpm.exact ? 'точно из Groq 429' : hasTpmHeader ? `Groq header · reset ${shortReset(rate.resetTokens)}` : 'локальный rolling 60s'),
    metricRow('TPD · токены/день', tpd.used, tpd.limit, tpd.exact ? 'точно из Groq 429' : telemetry.day?.seededFromApiCost ? 'подтянут расход Workbench за сегодня' : 'локально в этом Chrome-профиле'),
    metricRow('RPM · запросы/мин', rpm.used, rpm.limit, rpm.exact ? 'точно из Groq 429' : 'локальный rolling 60s'),
    metricRow('RPD · запросы/день', rpd.used, rpd.limit, rpd.exact ? 'точно из Groq 429' : hasRpdHeader ? `Groq header · reset ${shortReset(rate.resetRequests)}` : 'локально в этом Chrome-профиле')
  );
...
function ensureRoot() {
  ...
  const anchor = document.querySelector('.ai-lab-cost') || document.getElementById('aiLabUsage') || document.getElementById('aiLabIdentity');
...
async function render() {
  ...
  subtitle.textContent = 'TPM/RPD — из Groq headers; TPD/RPM — локально, а при 429 заменяются точными Used/Limit провайдера.';
