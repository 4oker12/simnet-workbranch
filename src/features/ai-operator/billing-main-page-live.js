...
async function executeRead(tabId, id) {
  const [execution] = await chrome.scripting.executeScript({
    ...
    func: async targetBillingId => {
      ...
      const parseMain = root => {
        const tariffDisplay = rowValue(root, [/^тарифи\s+на\s+інтернет/i, /^тарифы\s+на\s+интернет/i]);
        const tariffMatch = tariffDisplay.match(/^\[(\d+)\]\s*(.+)$/);
        const selectedTariff = selected(root, 'paket');
        const currentTariff = compact(tariffMatch?.[2] || tariffDisplay || selectedTariff || '', 260);
        const tariffId = compact(tariffMatch?.[1] || '', 40);
        const nextTariffNode = root?.querySelector?.('select[name="next_paket"]');
        const nextTariff = nextTariffNode ? selected(root, 'next_paket') : null;
        const nextTariffDelay = selected(root, 'next_paket_delay');
        return {
          identity: {
          ...
          finance: {
            accountBalance: money(rowValue(root, [/^на\s+счету,?\s*грн/i, /^на\s+рахунку,?\s*грн/i])),
            price: money(rowValue(root, [/^ціна,?\s*грн/i, /^цена,?\s*грн/i])),
            totalDue: money(rowValue(root, [/^разом\s+до\s+сплати/i, /^итого\s+к\s+оплате/i])),
            balanceAfterTariff: money(rowValue(root, [
              /на\s+счете\s+с\s+учетом\s+стоимости\s+тарифного\s+плана/i,
              /на\s+рахунку\s+з\s+урахуванням\s+вартості\s+тарифного\s+плану/i
            ])),
            balanceWithoutTemporary: money(rowValue(root, [
              /на\s+счете\s+без\s+учета\s+временных\s+платежей/i,
              /на\s+рахунку\s+без\s+урахування\s+тимчасових\s+платежів/i
            ])),
            priceSemantics: 'internet_tariff_price_from_main_page',
            totalDueSemantics: 'current_total_due_from_main_page_not_future_charge'
          }
        };
      };
      const pageBillingId = () => {
      ...
      if (pageBillingId() === String(targetBillingId)) {
        return { ok: true, code: 'OK', data: parseMain(document), transport: 'dom', endpoint: 'current-document' };
      }
      ...
      if (!response.ok) return { ok: false, code: 'BILLING_MAIN_PAGE_FETCH_FAILED', status: response.status };
      if (authPage(doc)) return { ok: false, code: 'BILLING_AUTH_REQUIRED' };
      return { ok: true, code: 'OK', data: parseMain(doc), transport: 'fetch', endpoint: '/cgi-bin/adm/adm.pl?a=user&id=<billingId>' };
    }
  });
  return execution?.result || { ok: false, code: 'BILLING_MAIN_PAGE_NO_RESULT' };
}

export async function readBillingMainPageLive({ billingId: rawBillingId, refresh = false, maxAgeMs = 120000 } = {}) {
    ...
    try {
      ...
      if (outcome?.ok) {
        const result = {
          ...
          source: 'billing-main-page-live-read-only',
          cache: 'miss'
        };
      ...
      last = { ok: false, code: 'BILLING_MAIN_PAGE_EXECUTION_FAILED', message: clean(error?.message || error, 500) };
    }
  }
  return { ...(last || { ok: false, code: 'BILLING_MAIN_PAGE_READ_FAILED' }), billingId: id, source: 'billing-main-page-live-read-only' };
}
