...
#### Semantic identity → literal validation

Формулировка вокруг идентификатора не обязана совпадать с regex посимвольно. Первый semantic-этап может понять, что клиент сообщает договор/особовой счёт/login даже при опечатке, суржике или необычном склонении, например:

```text
це мій особов рахонок 2421
особового рахунку 2421
мой логен Talala
```
...
LLM semantic understanding
  ...
  → предлагает ids.contract / ids.login
  ↓
subscriber-identity
  → проверяет, что само значение буквально присутствует в сообщении клиента
  ↓
customer.lookup
```

LLM не является evidence для значения идентификатора. Если semantic stage предложил `contract=5555`, а в исходной реплике есть только `2421`, hint отклоняется. Код не исправляет и не реконструирует номер договора. Очевидные формы (`2421`, `договор 2421`, `abon123456`) по-прежнему разбираются детерминированно без зависимости от LLM.

Это правило специально разделяет **гибкое понимание языка** и **детерминированное значение ID**: добавлять regex для каждого склонения или опечатки не требуется.
